...
export default function LabReportPage() {
  ...
  // Build a tight tweet-sized blurb. Prefer the LLM's ``share_blurb`` but
  // fall back to headline + first sentence of summary so the share buttons
  // still produce good copy when the LLM is quota-throttled.
  const shareText = useMemo(() => {
    if (!report) return "";
    const blurb = report.narrative.share_blurb?.trim();
    if (blurb) return blurb;
    const head = report.narrative.headline?.trim() ?? "";
    const sum =
      report.narrative.summary?.trim().split(/(?<=[.!?])\s+/)[0] ?? "";
    return [head, sum].filter(Boolean).join(" · ");
  }, [report]);
  ...
  const shareOn = useCallback(
    (platform: "x" | "telegram" | "whatsapp") => {
      const url = getShareUrl();
      const tag = "\n\nGenerated by MemeLab. AI-composed meme DNA intelligence.";
      // Twitter enforces ~280 chars; reserve ~25 for the URL + t.co wrapping.
      const tweetBody = `${shareText}${tag}`.slice(0, 255);
      const encText = encodeURIComponent(tweetBody);
      const encTextShort = encodeURIComponent(shareText);
      const encUrl = encodeURIComponent(url);
      let target = "";
      if (platform === "x") {
        target = `https://twitter.com/intent/tweet?text=${encText}&url=${encUrl}`;
      } else if (platform === "telegram") {
        target = `https://t.me/share/url?url=${encUrl}&text=${encTextShort}`;
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-xs text-white/55">
                    Post the AI findings to X, send a deep link, or export a PDF.
                  </div>
                </div>
              </div>
